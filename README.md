...
16. [EXE 빌드 (Windows)](#exe-빌드-windows)
16. [Windows 포트 연결 설정](#windows-포트-연결-설정)
17. [장비 통신 사양](#장비-통신-사양)
...
| 모델 | Baud | 데이터 비트 | Parity | Stop | 흐름 제어 |
|------|------|-----------|--------|------|----------|
| CA-410 | 38400 | 7 | EVEN | 2 | RTS/CTS |
| CA-310 | 9600  | 8 | NONE | 1 | 없음 |

**연결 시퀀스 (CA-410)**:
1. `serial.Serial(port, **params)` 포트 열기
...
- 응답 형식 2 (Compact): `OK00,Pxx v1;v2;v3\r`
- 저휘도(< 0.01 cd/m²) 시 응답 지연 → `timeout=5.0s`
...
| GET | `/api/export/<type>` | Excel 다운로드 |
...
pip install flask pyserial openpyxl
...
CA-310: 9600 / 8N1 / 흐름 제어 없음 → 칩셋 제한 없음.
